...
const { CURRENT_CONNECTOR_NAME, DEV_CONNECTOR_NAME, DEV_LONG_WAIT_CONNECTOR_NAME } = require("../electron/connector-identity.cjs");
...
test("DEV long-wait probe keeps its dedicated connector identity", () => {
  const fixture = devHostFor({
    purpose: "dev-harness",
    mode: "full",
    appName: DEV_LONG_WAIT_CONNECTOR_NAME,
  });
  assert.equal(fixture.host.setupConnectorName(), DEV_LONG_WAIT_CONNECTOR_NAME);
});
