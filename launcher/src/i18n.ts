...
const en = {
  ...
  manualMcpStepThreeBody: "Create the separate connector shown below with the exact name Codex Zero Risk2 and select it yourself in every Zero Risk turn. Zero Risk requires its own tunnel and credentials, separate from Automatic mode. The launcher never inspects the ChatGPT DOM in this mode.",
  connectorMigrationNotice: "Upgrading from Codex Native? Leave the old connector untouched and create Codex Native3 as a new connector. Do not rename or refresh Codex Native. Verify runtime rejects a legacy-only setup.",
  manualConnectorNotice: "Codex Zero Risk2 is a separate connector. Select it yourself before sending every Zero Risk prompt; the launcher intentionally does not inspect ChatGPT to verify that choice.",
  ...
  zeroRiskSentConfirmation: "Require Sent confirmation",
  zeroRiskSentConfirmationBody: "After sending a Zero Risk prompt in ChatGPT, require a second confirmation in the Launcher. Turn this off to let the Codex Zero Risk2 connector confirm the turn automatically.",
  ...
  delegatedToolAuthority: "Delegated tool authority",
  delegatedToolAuthorityBody: "For tool-capable turns, delegate filesystem, cwd, network, sandbox, and approval authority to the outer Codex harness. Local installs use verified-environment by default. When the runtime is initialized, changing this setting restarts it.",
  delegatedToolAuthorityForcedEnvironment: "This value is forced by CODEX_CHATGPT_WEB_TOOL_AUTHORITY_MODE and cannot be changed here.",
  delegatedToolAuthorityForcedRemote: "This value is forced by the remote Responses deployment and cannot be changed here.",
  ...
  manualPromptInstructionAutomaticConfirmation: "The prompt is already in your clipboard. Paste it into the open ChatGPT tab, choose the model and effort you want, select the Codex Zero Risk2 connector, then send it. The connector will confirm this turn automatically.",
  manualPromptCopy: "Copy prompt",
  manualPromptCopied: "Copied",
  manualPromptCancel: "Cancel turn",
  manualPromptCancelConfirm: "Click again to cancel",
...
const zh: Record<keyof typeof en, string> = {
  ...
  manualMcpStepThreeBody: "请使用下方显示的准确名称 Codex Zero Risk2 创建独立连接器，并在每次引导式手动回合中自行选择它。Zero Risk 必须使用与自动模式分开的 Tunnel 和凭据。此模式不会执行 DOM 检查。",
  connectorMigrationNotice: "从 Codex Native 升级？请保留旧连接器不变，并新建名为 Codex Native3 的连接器。不要重命名或刷新 Codex Native。仅有旧连接器时，运行时验证会明确失败。",
  ...
  zeroRiskSentConfirmation: "需要已发送二次确认",
  zeroRiskSentConfirmationBody: "在 ChatGPT 中发送 Zero Risk 提示后，要求再回到启动器确认一次。关闭后，由 Codex Zero Risk2 连接器自动确认本回合。",
  manualBiggerContextUnavailable: "零风险模式已固定使用 3 倍压缩间隔，提示仍保持单段并由用户手动发送。",
  delegatedToolAuthority: "委托工具权限",
  delegatedToolAuthorityBody: "对可使用工具的回合，将 filesystem、cwd、network、sandbox 和 approval 的最终权限委托给外层 Codex Harness。本地安装默认使用 verified-environment。运行时已初始化时，切换此设置会重启运行时。",
  delegatedToolAuthorityForcedEnvironment: "该值由 CODEX_CHATGPT_WEB_TOOL_AUTHORITY_MODE 强制设置，不能在此修改。",
  delegatedToolAuthorityForcedRemote: "该值由远程 Responses 部署强制设置，不能在此修改。",
  ...
  manualPromptInstructionAutomaticConfirmation: "提示已复制到剪贴板。请将其粘贴到打开的 ChatGPT 标签页，选择所需模型和 effort，再选择 Codex Zero Risk2 连接器并发送。连接器会自动确认本回合。",
  manualPromptCopy: "复制提示",
  manualPromptCopied: "已复制",
  manualPromptCancel: "取消任务",
  manualPromptCancelConfirm: "再次点击取消",
...
const ja: Record<keyof typeof en, string> = {
  ...
  manualMcpStepThreeBody: "下に表示される正確な名前 Codex Zero Risk2 で専用コネクタを作成し、ガイド付き手動ターンごとに自分で選択してください。Zero Risk では、自動モードとは別の Tunnel と認証情報が必要です。このモードでは DOM 検査を行いません。",
  connectorMigrationNotice: "Codex Native からアップグレードする場合は、古いコネクタを変更せず、新しい Codex Native3 コネクタを作成してください。Codex Native の名前変更や更新は行わないでください。古いコネクタしかない場合、ランタイム検証は明示的に失敗します。",
  ...
  zeroRiskSentConfirmation: "送信済み確認を必須にする",
  zeroRiskSentConfirmationBody: "ChatGPT で Zero Risk のプロンプトを送信した後、ランチャーでもう一度確認します。オフにすると Codex Zero Risk2 コネクタがターンを自動確認します。",
  manualBiggerContextUnavailable: "ゼロリスクでは固定の 3 倍コンパクション間隔を使用します。プロンプトは 1 パートのまま手動で送信します。",
  delegatedToolAuthority: "ツール権限を委任",
  delegatedToolAuthorityBody: "ツールを使用するターンでは、filesystem、cwd、network、sandbox、approval の最終権限を外側の Codex Harness に委任します。ローカル環境の既定値は verified-environment です。ランタイムが初期化済みの場合、この設定を変更するとランタイムが再起動します。",
  delegatedToolAuthorityForcedEnvironment: "この値は CODEX_CHATGPT_WEB_TOOL_AUTHORITY_MODE によって強制されているため、ここでは変更できません。",
  delegatedToolAuthorityForcedRemote: "この値はリモート Responses デプロイによって強制されているため、ここでは変更できません。",
  ...
  manualPromptInstructionAutomaticConfirmation: "プロンプトはクリップボードにコピー済みです。開いている ChatGPT タブに貼り付け、モデルと effort、Codex Zero Risk2 コネクタを選んで送信してください。コネクタがこのターンを自動確認します。",
  manualPromptCopy: "プロンプトをコピー",
  manualPromptCopied: "コピーしました",
  manualPromptCancel: "ターンをキャンセル",
  manualPromptCancelConfirm: "もう一度押してキャンセル",
...
const ko: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "아래의 정확한 DEV 이름으로 별도의 커넥터를 만드세요. Codex Native3는 변경하지 않아 프로덕션 환경과 저장소 개발 환경을 동시에 사용할 수 있도록 유지하세요.",
  ...
  manualMcpStepThreeBody: "아래에 표시된 정확한 이름인 Codex Zero Risk2로 별도의 커넥터를 만들고, 모든 Zero Risk 턴에서 직접 선택하세요. Zero Risk는 자동 모드와 별도의 터널 및 인증 정보가 필요합니다. 이 모드에서는 런처가 ChatGPT DOM을 검사하지 않습니다.",
  connectorMigrationNotice: "Codex Native에서 업그레이드하는 경우 기존 커넥터는 그대로 두고 Codex Native3라는 새 커넥터를 만드세요. Codex Native의 이름을 변경하거나 새로고침하지 마세요. 기존 커넥터만 있는 구성은 런타임 검증에서 거부됩니다.",
  manualConnectorNotice: "Codex Zero Risk2는 별도의 커넥터입니다. 각 Zero Risk 프롬프트를 보내기 전에 직접 선택하세요. 런처는 해당 선택을 확인하기 위해 ChatGPT DOM을 의도적으로 검사하지 않습니다.",
  ...
  zeroRiskSentConfirmation: "전송 확인 필요",
  zeroRiskSentConfirmationBody: "ChatGPT에서 Zero Risk 프롬프트를 전송한 뒤 런처에서 한 번 더 확인합니다. 끄면 Codex Zero Risk2 커넥터가 턴을 자동으로 확인합니다.",
  manualBiggerContextUnavailable: "Zero Risk는 이미 고정된 3배 컴팩션 간격을 사용합니다. 프롬프트는 하나의 메시지로 유지되며 직접 전송합니다.",
  delegatedToolAuthority: "도구 권한 위임",
  delegatedToolAuthorityBody: "도구를 사용하는 턴에서 filesystem, cwd, network, sandbox, approval의 최종 권한을 바깥쪽 Codex Harness에 위임합니다. 로컬 설치의 기본값은 verified-environment입니다. 런타임이 초기화된 경우 이 설정을 변경하면 런타임이 다시 시작됩니다.",
  delegatedToolAuthorityForcedEnvironment: "이 값은 CODEX_CHATGPT_WEB_TOOL_AUTHORITY_MODE에 의해 강제되므로 여기에서 변경할 수 없습니다.",
  delegatedToolAuthorityForcedRemote: "이 값은 원격 Responses 배포에 의해 강제되므로 여기에서 변경할 수 없습니다.",
  manualPromptTitle: "이 턴을 ChatGPT에서 전송",
  manualPromptInstruction: "프롬프트가 이미 클립보드에 복사되어 있습니다. 열려 있는 ChatGPT 탭에 붙여넣고 원하는 모델과 effort를 선택한 다음 Codex Zero Risk2 커넥터를 선택하여 전송하고 아래에서 확인하세요.",
  manualPromptInstructionAutomaticConfirmation: "프롬프트가 이미 클립보드에 복사되어 있습니다. 열려 있는 ChatGPT 탭에 붙여넣고 원하는 모델과 effort, Codex Zero Risk2 커넥터를 선택한 뒤 전송하세요. 커넥터가 이 턴을 자동으로 확인합니다.",
  manualPromptCopy: "프롬프트 복사",
  manualPromptCopied: "복사됨",
  manualPromptCancel: "턴 취소",
  manualPromptCancelConfirm: "다시 눌러 취소",
  manualPromptSent: "전송됨",
  manualPromptWaiting: "Codex Zero Risk2 연결 대기 중",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "請使用下方正確的 DEV 名稱新增獨立連接器。保留 Codex Native3 不變，讓正式環境與儲存庫開發可以同時執行。",
  ...
  manualMcpStepThreeBody: "請使用下方顯示的正確名稱 Codex Zero Risk2 建立獨立連接器，並在每次引導式手動回合中自行選擇它。Zero Risk 必須使用與自動模式分開的 Tunnel 與憑證。此模式不會執行 DOM 檢查。",
  connectorMigrationNotice: "從 Codex Native 升級？請保留舊連接器不變，並新增名為 Codex Native3 的連接器。不要重新命名或重新整理 Codex Native。僅有舊連接器時，執行階段驗證會明確失敗。",
  manualConnectorNotice: "Codex Zero Risk2 是獨立連接器。每次傳送引導式提示前都需要自行選擇它；啟動器有意不檢查 ChatGPT DOM 來驗證該選擇。",
  ...
  zeroRiskSentConfirmation: "需要已傳送二次確認",
  zeroRiskSentConfirmationBody: "在 ChatGPT 中傳送 Zero Risk 提示後，要求再回到啟動器確認一次。關閉後，由 Codex Zero Risk2 連接器自動確認本回合。",
  manualBiggerContextUnavailable: "零風險模式已固定使用 3 倍壓縮間隔，提示仍保持單段並由使用者手動傳送。",
  delegatedToolAuthority: "委派工具權限",
  delegatedToolAuthorityBody: "對可使用工具的回合，將 filesystem、cwd、network、sandbox 與 approval 的最終權限委派給外層 Codex Harness。本機安裝預設使用 verified-environment。執行環境已初始化時，切換此設定會重新啟動執行環境。",
  delegatedToolAuthorityForcedEnvironment: "此值由 CODEX_CHATGPT_WEB_TOOL_AUTHORITY_MODE 強制設定，無法在此修改。",
  delegatedToolAuthorityForcedRemote: "此值由遠端 Responses 部署強制設定，無法在此修改。",
  manualPromptTitle: "在 ChatGPT 中傳送此回合",
  manualPromptInstruction: "提示已複製到剪貼簿。請將其貼到開啟的 ChatGPT 分頁，選擇所需模型與 effort，再選擇 Codex Zero Risk2 連接器，傳送後在下方確認。",
  manualPromptInstructionAutomaticConfirmation: "提示已複製到剪貼簿。請將其貼到開啟的 ChatGPT 分頁，選擇所需模型與 effort，再選擇 Codex Zero Risk2 連接器並傳送。連接器會自動確認本回合。",
  manualPromptCopy: "複製提示",
  manualPromptCopied: "已複製",
  manualPromptCancel: "取消任務",
  manualPromptCancelConfirm: "再次點擊取消",
  manualPromptSent: "已傳送",
  manualPromptWaiting: "等待 Codex Zero Risk2 連線",
